#!/usr/bin/env bash
# Wrapper script for Finzy. Works for both:
#   - Portable tarball (run from extracted directory)
#   - System packages (/usr/bin/finzy -> /opt/dkmcgowan/finzy/)

# Resolve the real path of this script, following symlinks
SCRIPT_PATH="$(readlink -f "$0")"
INSTALL_DIR="$(dirname "$SCRIPT_PATH")"

# If we're in /usr/bin, the actual install is in /opt/dkmcgowan/finzy
if [[ "$INSTALL_DIR" == "/usr/bin" ]]; then
    INSTALL_DIR="/opt/dkmcgowan/finzy"
fi
