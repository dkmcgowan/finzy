...
while true; do
  ...
  done < <({ ldd "$BUNDLE/finzy" 2>/dev/null; find "$BUNDLE_LIB" -name '*.so*' -type f -exec ldd {} \; 2>/dev/null; } | grep "=> /" | awk '{print $3}' | sort -u)
...
MISSING=$({ LD_LIBRARY_PATH="$BUNDLE_LIB" ldd "$BUNDLE/finzy" 2>&1; find "$BUNDLE_LIB" -name '*.so*' -type f -exec env LD_LIBRARY_PATH="$BUNDLE_LIB" ldd {} \; 2>&1; } | grep "not found" | sort -u || true)
if [[ -n "$MISSING" ]]; then
...
mv "$BUNDLE/finzy" "$BUNDLE/finzy.bin"
cat > "$BUNDLE/finzy" <<'LAUNCHER'
#!/usr/bin/env bash
...
exec "$INSTALL_DIR/finzy.bin" "$@"
LAUNCHER
chmod +x "$BUNDLE/finzy"

echo ""
echo "==> Creating tarball..."
cd "$BUNDLE"
tar -czf /app/finzy-steam-linux-x64.tar.gz *

echo ""
echo "==> Done! Output: finzy-steam-linux-x64.tar.gz"
